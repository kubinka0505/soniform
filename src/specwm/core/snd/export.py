...
def convert(
	...
	Logger.info(f"\tduration           : {cfg.duration:.3f}s")
	Logger.info(f"\tsample rate        : {cfg.sample_rate} Hz")
	Logger.info(f"\tn_fft | hop length : {cfg.n_fft} | {cfg.hop_length}")
	Logger.info(f"\tfrequency band     : {cfg.resolved_freq_min} to {cfg.resolved_freq_max} Hz ({cfg.frequency_mode})")
	Logger.info(f"\tgriffin-lim        : {cfg.griffin_lim_iters} iterations")